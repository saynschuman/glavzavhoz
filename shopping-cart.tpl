...
<script type="text/javascript">
	...
	var cartBlockWidth = $('.cart-info-block-inner').width();
	var goodsPosition = $('.cart-table').offset().top + $('.cart-table').height() - $('.cart-info-block-inner').height()-70;
	// console.log('goodsPosition: '+goodsPosition);
	// console.log('$(".cart-table").offset().top: '+$('.cart-table').offset().top);
	// console.log('$(".cart-table").height(): '+$('.cart-table').height());
	// console.log('$(".cart-info-block-inner").height(): '+$('.cart-info-block-inner').height());
	$(window).scroll(function () {
		// console.log($(this).scrollTop());
		goodsPosition = $('.cart-table').offset().top + $('.cart-table').height() - $('.cart-info-block-inner').height()-70;
	if ($(this).scrollTop() > 200 && $(this).scrollTop() < goodsPosition) {
	    $('.cart-info-block-inner').addClass("fixed");
	    $('.cart-info-block-inner').width(cartBlockWidth);
	    $('.cart-info-block-inner').css("margin-top",'0');
	} else if ($(this).scrollTop() >= goodsPosition){
	    $('.cart-info-block-inner').removeClass("fixed");
	    $('.cart-info-block-inner').css("margin-top",$('.cart-table').height()-$('.cart-info-block-inner').height());
	} else {
	    $('.cart-info-block-inner').removeClass("fixed");
	    $('.cart-info-block-inner').width('auto');
	    $('.cart-info-block-inner').css("margin-top",'0');
	}
	});
});
...
<div class="box col-md-8 cart-table">
		...
		<table id="cart_summary" class="table table-bordered {if $PS_STOCK_MANAGEMENT}stock-management-on{else}stock-management-off{/if}">
			<thead>
				<tr>
					...
					<th class="cart_unit item text-center">Цена</th>
				</tr>
			</thead>
			<tbody>
...
<div class="col-md-4 cart-info-block">
	<div class="cart-info-block-inner">
	<h3>Ваш заказ</h3>
	...
	{if $use_taxes}
		{if $priceDisplay}
			<tr class="cart_total_price">
				...
				<td colspan="2"><span class="price" id="total_product">{displayPrice price=$total_products}</span></td> -->
			</tr>
			<div class="row">
				<div class="col-md-6">На сумму:</div>
				<div class="col-md-6 text-right"><span class="price" id="total_product">{displayPrice price=$total_products}</span></div>
			</div>
		{else}
			<tr class="cart_total_price">
				...
				<!-- <td colspan="2" class="price" id="total_product"><span class="price" id="total_product">{displayPrice price=$total_products_wt}</span></td> -->
			</tr>
			<div class="row cart_total_price">
				<div class="col-md-6">На сумму:</div>
				<div class="col-md-6 text-right"><span class="price" id="total_product">{displayPrice price=$total_products_wt}</span></div>
			</div>
		{/if}
	{else}
		<tr class="cart_total_price">
			...
			<td colspan="2" class="price" id="total_product"><span class="price" id="total_product">{displayPrice price=$total_products}</span></td>
		</tr>
	{/if}
	...
	{*{if $total_shipping_tax_exc <= 0 && (!isset($isVirtualCart) || !$isVirtualCart) && $free_ship}*}
		...
		{*</tr> -->*}
		{*<div class="row" cart_total_delivery>*}
			{*<div class="col-md-4">Доставка:</div>*}
			{*<div class="col-md-8 text-right"><span class="price" id="total_shipping">{l s='Free Shipping!'}</span></div>*}
		{*</div>*}
	{*{else}*}
		{*{if $use_taxes && $total_shipping_tax_exc != $total_shipping}*}
			{*{if $priceDisplay}*}
				...
				{*<div class="row cart_total_delivery">*}
					{*<div class="col-md-4">Доставка:</div>*}
					{*<div class="col-md-8 text-right"><span class="price" id="total_shipping">{displayPrice price=$total_shipping_tax_exc}</span></div>*}
				{*</div>*}
			{*{else}*}
				...
				{*<div class="row cart_total_delivery">*}
					{*<div class="col-md-4">Доставка:</div>*}
					{*<div class="col-md-8 text-right"><span class="price" id="total_shipping">{displayPrice price=$total_shipping}</span></div>*}
				{*</div>*}
			{*{/if}*}
		{*{else}*}
			{*<!-- <tr class="cart_total_delivery{if $total_shipping_tax_exc <= 0} unvisible{/if}">*}
	...
	<div class="row cart_total_price">
		<div class="col-md-4 class="total_price_container text-right""><span>{l s='Total'}:</span></div>
		{*{if $use_taxes}*}
			{*<div class="col-md-8 text-right" id="total_price_container"><span class="price" id="total_price">{displayPrice price=$total_price}</span></div>*}
		{*{else}*}
			{*<div class="col-md-8 text-right" id="total_price_container"><span class="price" id="total_price">{displayPrice price=$total_price_without_tax}</span></div>*}
		{*{/if}*}
        {if $use_taxes}
        <div class="col-md-8 text-right" id="total_price_container"><span class="price" id="total_price">{displayPrice price=$total_products}</span></div>
        {else}
        <div class="col-md-8 text-right" id="total_price_container"><span class="price" id="total_price">{displayPrice price=$total_products}</span></div>
        {/if}
