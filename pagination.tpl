...
{if isset($p) AND $p}
	...
	{else}
		{if !isset($current_url)}
			...
			{*newed_log getPaginationLink выше возвращал ссылку без ЧПУ *}
			{assign var='requestPage' value=$link->getPaginationLink('category', $category, false, false, true, false)}
		{else}
			{assign var='requestPage' value=$current_url}
		{/if}
	...
	{/if}
	    {if $nb_products > $products_per_page && $start!=$stop}
			<form class="showall" action="{if !is_array($requestNb)}{$requestNb|escape:'html':'UTF-8'}{else}{$requestNb.requestUrl|escape:'html':'UTF-8'}{/if}" method="get" data-anegve-requestPage="{$requestPage}">
				<div>
		...
		{if $start!=$stop}
			<ul class="list-unstyled pagination clearfix">
				...
				{section name=pagination start=$start loop=$stop+1 step=1}
					...
					{else}
						<li>
							{*<p>{$requestPage|var_dump}</p> newed_log link after redirect*}
