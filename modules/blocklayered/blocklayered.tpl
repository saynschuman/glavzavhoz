...
{if $nbr_filterBlocks != 0}
    <div id="layered_block_left" class="block block-borderbox">
        <!-- <p class="title_block"><span>{l s='filters Catalog' mod='blocklayered'}</span></p> -->
        <div class="block_content box-content">
            <form action="#" id="layered_form">
                <div>
                                        ...
                                        {if $filter.type == $filter_type && isset($filter.values)}
                                            {if isset($filter.slider)}
                                                {if $smarty.foreach.f_values.first}
                                                    <li class="layred_filter-name">
                                                        <a href="#" rel="layered_{$filter.type}_slider"
                                            ...
                                            {else}
                                                {foreach from=$filter.values key=id_value item=value}
                                                    {if $id_value == $filter_key && !is_numeric($filter_value) && ($filter.type eq 'id_attribute_group' || $filter.type eq 'id_feature') || $id_value == $filter_value && $filter.type neq 'id_attribute_group' && $filter.type neq 'id_feature'}
                                                        <li class="layred_filter-name">
                                                            <a class="layered_remove"
                                                               href="#"
                                                               rel="layered_{$filter.type_lite}_{$id_value}"
                                                               title="{l s='Cancel' mod='blocklayered'}"></a>
                                                               <span>
                                                                   {l s='%1$s' mod='blocklayered' sprintf=[$filter.name]}:
                                                               </span>
                    ...
                    {if isset($filter.slider)}
                    <div class="layered_all layered_{$filter.type} js-slide__trg closed"> <!--style="display: none;"-->
                        {else}
                        <div class="layered_all layered_filter js-slide__trg closed">
                            {/if}
                            <div class="layered_subtitle_heading js-slide">
                            ...
                            <ul id="ul_layered_{$filter.type}_{$filter.id_key}"
                                class="col-lg-12 layered_filter_ul{if isset($filter.is_color_group) && $filter.is_color_group} color-group{/if}" style="display: none;">
                                {if !isset($filter.slider)}
                                    {if $filter.filter_type == 0}
                                        {foreach from=$filter.values key=id_value item=value name=fe}
                                            {if $value.nbr || !$hide_0_values}
                                                <li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if} rkmd-checkbox checkbox-rotate checkbox-ripple">
                                                    {if isset($filter.is_color_group) && $filter.is_color_group}
                                                        ...
                                                        {if isset($value.checked) && $value.checked}<input type="hidden"
                                                               name="layered_{$filter.type_lite}_{$id_value}"
                                                               value="{$id_value}" />{/if}
                                                        {else}
                                                        <label class='input-checkbox checkbox-lightBlue'><input type="checkbox"
                                                              class="checkbox"
                                                              name="layered_{$filter.type_lite}_{$id_value}"
                                                              ...
                                                              <span class='checkbox-mod'></span> </label>
                                                    {/if}
                                                    <label for="layered_{$filter.type_lite}_{$id_value}"{if !$value.nbr} class="disabled"{else}{if isset($filter.is_color_group) && $filter.is_color_group} name="layered_{$filter.type_lite}_{$id_value}" class="layered_color" rel="{$id_value}_{$filter.id_key}"{/if}{/if}>
                                                        ...
                                                        {else}
                                                            ...
                                                            {*<a href="{$value.link}"*}
                                                                    {*{if $value.rel|trim != ''} rel="{$value.rel}"{/if}>*}
                                                                {*{$value.name|escape:'html':'UTF-8'}{if $layered_show_qties}*}
                                                                    {*<span> ({$value.nbr})</span>*}
                                                                {*{/if}*}
                                                            {*</a>*}
                                            ...
                                            {foreach from=$filter.values key=id_value item=value name=fe}
                                                {if $value.nbr || !$hide_0_values}
                                                    <li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if}">
                                                        {if isset($filter.is_color_group) && $filter.is_color_group}
                                                            <input class="radio color-option {if isset($value.checked) && $value.checked}on{/if} {if !$value.nbr}disable{/if}"
                                                                   type="button"
                                                                   name="layered_{$filter.type_lite}_{$id_value}"
                                                                   rel="{$id_value}_{$filter.id_key}"
                                                                   id="layered_id_attribute_group_{$id_value}"
                                                        ...
                                                        {else}
                                                            <input type="radio"
                                                                   class="radio layered_{$filter.type_lite}_{$id_value}"
                                                        ...
                                                        <label for="layered_{$filter.type_lite}_{$id_value}"{if !$value.nbr} class="disabled"{else}{if isset($filter.is_color_group) && $filter.is_color_group} name="layered_{$filter.type_lite}_{$id_value}" class="layered_color" rel="{$id_value}_{$filter.id_key}"{/if}{/if}>
                                                            {if !$value.nbr}
                                                                ...
                                                                <span> ({$value.nbr})</span>{/if}
                                                            {else}
                                                                <a href="{$value.link}"{if $value.rel|trim != ''} rel="{$value.rel}"{/if}>{$value.name|escape:'html':'UTF-8'}{if $layered_show_qties}
                                ...
                                {else}
                                    {if $filter.filter_type == 0}
                                        ...
                                        <div class="layered_slider_container">
                                            <div class="slider slider-material-orange js-nouislider"
                                                 data-type="{$filter.type}" data-format="{$filter.format}"
                                                 data-unit="{$filter.unit}"></div><!--id="layered_{$filter.type}_slider" -->
                                        </div>
                                    {else}
                                        {if $filter.filter_type == 1}
                                            <li class="nomargin row">
                                                <div class="col-xs-6 col-sm-12 col-lg-6 first-item">
                                                    ...
                                                    <input class="layered_{$filter.type}_range layered_input_range_min layered_input_range form-control grey"
                                                ...
                                                <div class="col-xs-6 col-sm-12 col-lg-6">
                                                    ...
                                                    <input class="layered_{$filter.type}_range layered_input_range_max layered_input_range form-control grey"
